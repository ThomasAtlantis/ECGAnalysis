...
def cross_valid():
    ...
    # models = (svm.LinearSVC(),) # won't converge
    models = (svm.SVC(kernel='linear'), svm.SVC(kernel='poly', degree=3), svm.SVC(kernel='rbf', gamma='scale'))
    for model in models:
        accuracies = []
...
def cross_valid_new():
    for model in (svm.SVC(kernel='rbf', gamma=g, C=1) for g in np.linspace(0.1, 0.6, 11)):
        accuracies = []
        for fold_index in range(min(process_data.FOLD, 10)):
            X_valid, y_valid = dataset[fold_index]
            X_valid = X_valid.astype('float32')
